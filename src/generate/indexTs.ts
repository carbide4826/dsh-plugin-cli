// 【M2-c】入口与聚合文件生成器:index.ts / events.ts / seams/index.ts / client/index.ts。
// 文件骨架由本模块渲染,行来自 GenerationPlan(模板里的同名聚合文件只是全勾参考,不参与拷贝)。
import type { Answers } from "../domain/types";
import type { AggregatePlan } from "../atoms";

/** index 拼装行(与 GenerationPlan.index 同形) */
...
/** Config 声明块(⑤ static/dynamic):interface + Schemastery schema */
...
export function generateHostIndex(answers: Answers, index: IndexBlock): string {
    ...
    lines.push(`// ${answers.pkgName} — 插件入口(由 dshp 按勾选拼装生成)。`);
    ...
    if (dynamic) {
        lines.push("import type {} from '@deepseek-ai/dsh-settings' // ctx.settings 的类型来源(动态配置)");
    }
    ...
    if (index.injects.length > 0 || dynamic) {
        ...
        const injects = dynamic ? [...index.injects, "settings"] : index.injects;
        lines.push("");
        lines.push("// 要求就绪的服务(决定加载顺序)");
        lines.push(`export const inject = [${injects.map((s) => `'${s}'`).join(", ")}]`);
    }
    ...
    lines.push("");
    lines.push("/**");
    lines.push(" * 插件入口:各原子的注册调用(由 dshp 按勾选拼装)。");
    lines.push(" * @param ctx - Cordis 上下文");
    if (answers.config !== "none") lines.push(" * @param config - 已解析的插件配置");
    lines.push(" */");
    if (answers.config === "none") {
        lines.push("export function apply(ctx: Context): void {");
    } else {
        lines.push("export function apply(ctx: Context, config: Config): void {");
    }
    if (index.calls.length === 0) {
        ...
        if (answers.config !== "none") lines.push("    void config // TODO: 把配置接进你的实现");
    }
    if (dynamic) {
        lines.push("    // 动态配置:注册 settings section;部署侧 provider(dsh-settings-file)变更时热更新");
        lines.push("    let getSource: () => Config = () => config");
        lines.push("    ctx.settings.installSection(ctx, '" + answers.pluginId + "', Config, config, {");
        lines.push("        setSource: (source) => {");
        lines.push("            getSource = source");
        lines.push("        },");
        lines.push("        onChange: () => {");
        lines.push("            const current = getSource()");
        lines.push("            void current // TODO: 响应配置变化(如热更新 API key)");
        lines.push("        },");
        lines.push("    })");
    }
    lines.push("}");
