...
带持久化的用户便签本:storage 缝注册自定义后端,NotebookService 封装 KvUnit,`note` 工具让模型替用户记/查/列笔记。对标社区 dsh-project-memory 的"记忆插件"形态(对标本案例用的是内存介质,换 json/sqlite 官方后端或自写介质即得持久化)。

## 关键文件导览

| 文件 | 看什么 |
|---|---|
| `src/seams/storage.ts` | StorageBackend 最小实现(内存 KV):`kv.open` + 五方法的单元 |
| `src/service.ts` | NotebookService:`backend.get('notebook').kv.open(descriptor)` + 声明合并暴露 `ctx.notebook` |
| `src/tool.ts` | 单工具多动作(put/get/list)+ execute 闭包持 ctx 访问服务 |
...
3. 数据落在注册的 `notebook` 后端;把后端实现换成文件/SQLite(或部署侧选官方 json/sqlite 后端)即得真持久化
