...
Curated case: bridge external webhooks into the session (protocol + events)
...
　　启动后打印 Web 地址(默认 http://127.0.0.1:3080)即代表 apply 已执行;功能级验证走「如何验证起效」的 curl 投递链路(对话式验证不适用本案例)。
...
　　⑤ **自建 agent 的会话要持久化,必须 `workspace.attachSession`**(官方 `createWebhookSession` 同款链路):`workspaceRegistry.create(绝对路径)` → `agents.create({sessionId, meta:{cwd}})` → `workspace.attachSession(sessionId)`——缺最后一步,session 只在 UI 列表可见、点开报 `session/not-found`(数据没落盘)。同时 `agents.create` 必须带 `agentOptions`(本案例从 `agentDefaultModel.currentSelection()` 取默认模型),缺了它 agent 跑一轮时报 `prompt variable "{{model}}" has no value`。
...
　　演示"外部事件桥接":插件起一个 HTTP webhook,收到的文本被作为插件来源消息投进会话跑一轮 agent(protocol 原子开自有资源 + `ctx.effect` 管生命周期 + Config 出端口)。

### 关键文件导览

| 文件 | 看什么 |
|---|---|
| `src/protocol.ts` | HTTP webhook + `ctx.effect` 生命周期 + 会话落盘(见 tip 5) |
| `src/index.ts` | inject 三服务(agents / workspaceRegistry / agentDefaultModel)+ Config(port) |
| `src/domains/session.ts` | session 域监听(打印 turn 流向) |

### 如何验证起效

1. 按「快速开始」1~4 步跑起来 → 启动日志出现 `[webhook-bridge] webhook listening on :8787`
2. 另开终端投递:`curl -X POST http://127.0.0.1:8787/hook -d '{"text":"hi"}'` → 返回 `{"accepted":true}`,终端打出 `session created`
3. Web UI 会话列表出现该会话,点开历史正常加载(见 tip 5)
