...
外部系统事件桥接:插件起一个 HTTP webhook(CI 通知、IM 推送都往这打),`POST /hook` 收到的文本被作为插件来源消息投进会话跑一轮 agent。演示"protocol 原子开自有资源 + ctx.effect 管生命周期 + Config 出端口"。

## 关键文件导览

| 文件 | 看什么 |
|---|---|
| `src/protocol.ts` | `createServer` + `ctx.effect`(卸载自动关停)+ `ctx.agents.create/followup` 投递 |
| `src/index.ts` | Config(port)静态配置:写进 cordis.yml,加载时生效 |
| `src/domains/session.ts` | session 域监听(桥接侧的会话事件观察位) |

## 如何验证起效

1. `pnpm install --ignore-workspace && pnpm build && dsh plugin add .`
2. 启动日志出现 **`[webhook-bridge] webhook listening on :8787`**
3. 另开终端:`curl -X POST http://127.0.0.1:8787/hook -d '{"text":"CI 构建完成 ✅"}'` → 返回 `{"accepted":true}`
4. 会话流出现 CI 消息并跑一轮回复;换端口:改 patch 里 `config.port` 重启(整行替换,见防坑第 6 条)
