...
// TODO: 移到 Config(Schemastery),不要硬编码端口
const PORT = 8787
 ...
 */
export function registerProtocol(ctx: Context): void {
    ...
    ctx.effect(() => {
        server.listen(PORT)
        console.log(`[{{PLUGIN_ID}}] webhook listening on :${PORT}`)
        return () => {
            server.close()
            console.log(`[{{PLUGIN_ID}}] webhook server closed`)
        }
    }, '{{PLUGIN_ID}}: webhook server')
